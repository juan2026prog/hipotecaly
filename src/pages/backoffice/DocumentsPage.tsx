...
export const DocumentsPage: React.FC = () => {
  const allDocs = DEMO_APPLICATIONS.flatMap((app) =>
    (app.documents || []).map((doc: any) => ({
      ...doc,
      appId: app.id,
      publicId: app.public_id,
      borrowerName: `${app.borrower.first_name} ${app.borrower.last_name}`,
    }))
  );
      ...
      <div className="space-y-6 text-left max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-muted mt-0.5">
              Repositorio de documentación privada con enlaces firmados temporales.
            </p>
          </div>
        ...
        <div className="bg-white rounded-card border border-slate-border shadow-card overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between text-xs font-bold text-navy">
            <span>Documentos Subidos ({allDocs.length})</span>
          </div>
                ...
                <div className="flex items-center space-x-3.5">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-[11px] mt-0.5">
                      {doc.file_name} · {(doc.file_size / (1024 * 1024)).toFixed(1)} MB · Solicitud{' '}
