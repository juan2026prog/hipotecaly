...
export const TasadorNewAppraisalPage: React.FC = () => {
  ...
  const [horizontalProperty, setHorizontalProperty] = useState<boolean | undefined>(undefined);
  ...
  const [toilettes, setToilettes] = useState<number | ''>('');
  const [garages, setGarages] = useState<number | ''>('');
  const [floorLevel, _setFloorLevel] = useState<number | ''>('');

  // Bloque E: Amenities (Tri-state / Sparse map: solo claves explícitamente informadas)
  const [amenities, setAmenities] = useState<Record<string, boolean | undefined>>({});
    ...
    const loadCanonicalProperty = async () => {
      ...
      try {
        ...
        if (propData) {
          ...
          if (propData.total_surface_m2 !== null && propData.total_surface_m2 !== undefined) {
            const t = Number(propData.total_surface_m2);
            if (!isNaN(t) && t > 0) setTotalAreaM2(t);
          } else if (propData.surface_m2 !== null && propData.surface_m2 !== undefined) {
            // LEGACY ONLY: si total_surface_m2 no existe históricamente
            const s = Number(propData.surface_m2);
            if (!isNaN(s) && s > 0) setTotalAreaM2(s);
          }

          if (propData.covered_surface_m2 !== null && propData.covered_surface_m2 !== undefined) {
            const c = Number(propData.covered_surface_m2);
            if (!isNaN(c) && c > 0) setCoveredAreaM2(c);
          }

          if (propData.built_surface_m2 !== null && propData.built_surface_m2 !== undefined) {
            const b = Number(propData.built_surface_m2);
            if (!isNaN(b) && b > 0) setBuiltAreaM2(b);
          }
  ...
  const toggleAmenity = (key: string) => {
    setAmenities((prev) => {
      ...
      if (current === undefined) {
        next[key] = true; // 1er click: Tiene (YES / true)
      } else if (current === true) {
        next[key] = false; // 2do click: No tiene (NO / false)
      } else {
        delete next[key]; // 3er click: Reset a No informado (UNKNOWN / undefined)
      }
      return next;
    });
  };
    ...
    try {
      ...
      if (canonicalPropertyId && isSupabaseConfigured) {
        try {
          ...
          if (typeof totalAreaM2 === 'number' && !isNaN(totalAreaM2)) enrichFieldProv('total_surface_m2', totalAreaM2);
          if (typeof builtAreaM2 === 'number' && !isNaN(builtAreaM2)) enrichFieldProv('built_surface_m2', builtAreaM2);
          if (typeof coveredAreaM2 === 'number' && !isNaN(coveredAreaM2)) enrichFieldProv('covered_surface_m2', coveredAreaM2);
          ...
          await supabase
            ...
            .update({
              ...
              latitude: geoAddress.latitude,
              longitude: geoAddress.longitude,
              total_surface_m2: typeof totalAreaM2 === 'number' && !isNaN(totalAreaM2) ? totalAreaM2 : null,
              built_surface_m2: typeof builtAreaM2 === 'number' && !isNaN(builtAreaM2) ? builtAreaM2 : null,
              covered_surface_m2: typeof coveredAreaM2 === 'number' && !isNaN(coveredAreaM2) ? coveredAreaM2 : null,
              ...
              surface_m2: typeof totalAreaM2 === 'number' && !isNaN(totalAreaM2) ? totalAreaM2 : (typeof coveredAreaM2 === 'number' && !isNaN(coveredAreaM2) ? coveredAreaM2 : null),
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
              ...
              <div className="flex items-center space-x-4 pt-2 text-xs">
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={horizontalProperty === true}
                    onChange={(e) => setHorizontalProperty(e.target.checked ? true : undefined)}
                ...
                {horizontalProperty !== undefined && (
                  <button
                    type="button"
                    onClick={() => setHorizontalProperty(undefined)}
                    className="text-[10px] text-slate-400 hover:text-slate-600 underline"
                  >
                    (Desmarcar / No informado)
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                ...
                <div>
                  ...
                  <select
                    value={toilettes}
                    onChange={(e) => setToilettes(e.target.value === '' ? '' : Number(e.target.value))}
                    className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#102d49]/20"
                  >
                ...
                <div>
                  ...
                  >
                    ...
                    {[0, 1, 2, 3, 4].map((num) => (
                      <option key={num} value={num}>
                        {num === 0 ? 'Sin garaje (0)' : `${num} garaje(s)`}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
              <div className="flex items-center space-x-2.5 pb-3 border-b border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">Servicios e infraestructura que inciden en el valor de mercado (click: Sí ✓, click: No ✗, click: No informado —)</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
                {[
                  ...
                  const stateVal = amenities[amenity.key];
                  return (
                    <button
                      key={amenity.key}
                      type="button"
              ...
              <div className="divide-y divide-slate-100 text-xs">
                ...
                <div className="py-2.5 flex justify-between">
                  ...
                  <span className="font-bold text-slate-800">
                    {garages !== '' ? (garages > 0 ? `${garages} garaje(s)` : 'Sin garaje (0)') : '—'}
                  </span>
                </div>
                <div className="py-2.5 flex justify-between">
