...
export class DiditKycProvider implements KycProvider {
  ...
  public async createSession(input: KycSessionInput): Promise<KycSession> {
    // Si estamos en modo mock explícito o no hay API key configurada, retornar sesión simulada
    if (!this.apiKey || this.mode === 'mock') {
    ...
    try {
      ...
      // Fallback a modo test/simulación si la red falla en desarrollo/QA
      if (this.mode === 'test' || process.env.NODE_ENV !== 'production') {
        const fallbackId = `didit_fallback_${Date.now()}`;
        return {
          id: fallbackId,
          sessionId: fallbackId,
          sessionUrl: `/mi-cuenta?didit_mock_session=${fallbackId}`,
          provider: 'didit',
          mode: 'test',
          status: 'created',
          createdAt: new Date().toISOString(),
